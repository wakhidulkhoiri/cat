...
return trim(substr($str,$start +$length,$end)); } function string($length = 15) { 	
$characters = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'; 	$charactersLength = strlen($characters); 	
$randomString = ''; 	
for ($i = 0; $i < $length; $i++) { 		
$randomString .= $characters[rand(0, $charactersLength - 1)]; 	} 	
return $randomString; } function angka($length = 15) { 	
$characters = '0123456789'; 	$charactersLength = strlen($characters); 	
$randomString = ''; 	
for ($i = 0; $i < $length; $i++) { 		
$randomString .= $characters[rand(0, $charactersLength - 1)]; 	} 	
return $randomString; } function coin($token) { 	$curl = new curl(); 	
$curl->cookies('cookies/'.md5($_SERVER['REMOTE_ADDR']).'.txt'); 	
$curl->ssl(0, 2); 	$page = $curl->post('https://www.veeuapp.com/v1.0/incentive/tasks?access_token='.$token.'',' { 	"locale":"in_ID", 	
"task_extra_info":"", 	
...
echo "SUCCESS | ".$coin."\n"; 		
flush(); 		
ob_flush(); 	} elseif (strpos($page, 'token invalid')) { 		
echo "TOKEN INVALID"; 		
echo "\n"; 		
flush(); 		ob_flush(); 	
...
=================================================== CREATE BY: Akbar Nur Ichfan 
================================================== \n"; echo "Masukkan Auth Token: "; $token = trim(fgets(STDIN)); if ($token == "")
 { 	die ("Action Token cannot be blank!\n"); } else { 	while (true) { 		
coin($token); 	} } ?>
